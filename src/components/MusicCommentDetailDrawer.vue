...
const fetchReplies = async (commentId: number) => {
  try {
    const response = await service.get(`/api/music-comments/${commentId}/replies`, {
      params: {
        page: 1,
        limit: 100 // 获取所有回复
      }
...
const handleDeleteReply = async (replyId: number) => {
  try {
    const response = await service.delete(`/api/admin/music-comments/${replyId}`)
    if (response.success) {
